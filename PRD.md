...
| Templates (video) | 🟢 | 17 motion prompts — 16 product-category-specific (cosmetics, lipstick, skincare, perfume, drinks, soda, energy drink, ice cream, chocolate, shoes, sneakers, clothing, watch/jewelry, food packaging, electronics, generic) plus 1 multi-shot "Product Marketing" campaign template (`category: campaign`) built to consume several of the user's own photos at once. Landing-page cards autoplay their preview clip. Video templates can declare extra reference-photo slots as either fixed/required (`image_slot_labels`) or optional/unstructured (`image_slots_optional` — reuses the freeform no-template multi-photo flow, for a prompt written to adapt to whatever's uploaded rather than needing a specific shot per numbered slot). The Video tab on `/templates` is locked behind Basic+ (matches `VIDEO_TIERS.standard.minPlan`) — a Free-plan visitor sees an upgrade prompt there instead of browsing prompts they can't yet generate from; landing-page video template cards link into that tab (`/templates?type=video&template=id`) rather than straight into the generator |
...
- **`templates`** — `id`, `name`, `template_type` (`production`/`universal`/`campaign`/`video`), `category`, `description`, `tags[]`, `prompt` (service-role only — revoked from `anon`/`authenticated`), `image_slot_labels[]` (fixed/required extra reference-photo slots for multi-image video templates), `image_slots_optional` (bool — optional/unstructured extra photos instead of fixed slots), `cover_image_url`, `preview_video_url`, `accent_color`, `is_pro`, `sort_order`
...
**`/api/me` client-side dedup:** the dashboard layout, `/account`, `/templates`, `/generate`, and the video generator each used to run their own independent `fetch("/api/me")` on mount — five redundant round trips for the same response on every navigation. `lib/hooks/use-me.ts` is a module-scoped shared cache (not a TTL — every mount still revalidates against the real, uncached endpoint above): the first consumer to mount in a session fetches; every other consumer renders that snapshot instantly and gets updates via a shared listener set, so navigating between dashboard pages no longer flashes a loading state or re-queries credits/plan/profile that another component already has. Nothing that decides a charge reads from this cache — same DB-source-of-truth rule as above.
