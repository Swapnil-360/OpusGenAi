...
export default function TemplatesPage() {
  return (
    <Suspense
      fallback={<div className="h-full" style={{ background: "#0f0404" }} />}
    >
      <TemplatesPageInner />
    </Suspense>
  );
...
function TemplatesPageInner() {
  ...
  useEffect(() => {
    ...
    if (match && (match.templateType !== "video" || canBrowseVideo))
      setPreview(match);
  ...
  const filtered = getTemplatesByCategory(byType, activeCategory).filter(
    (t) => {
  ...
  function applyTemplate(tpl: Template) {
    ...
    const destination =
      tpl.templateType === "video" ? "/tools/image-to-video" : "/generate";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] ml-9" style={{ color: W.muted }}>
              Professional visual styles — click any template to instantly apply
              it.
            </p>
          </div>
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{ color: isActive ? W.red : W.text }}
                >
                  {t.label}
                  <span
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 no-scrollbar">
          {categories.map((cat) => {
            const count =
              cat.id === "all"
                ? byType.length
                : byType.filter((t) => t.category === cat.id).length;
      ...
      <div className="flex-1 overflow-y-auto py-4">
        {activeType === "video" && !canBrowseVideo ? (
          ...
          <div className="flex flex-col items-center text-center py-20 px-6">
            ...
            <p className="text-sm font-bold" style={{ color: W.text }}>
              Video templates need Basic or Pro
            </p>
            <p className="text-xs mt-1 max-w-xs" style={{ color: W.muted }}>
              Upgrade to bring your product photos to life with AI-generated
              motion.
            </p>
            <button
              onClick={() => triggerUpgradeModal("basic")}
              className="mt-4 h-9 px-4 rounded-lg text-xs font-bold text-white cursor-pointer hover:opacity-90 transition-opacity"
          ...
          <div className="flex items-center justify-center py-20">
            <div
              ...
              style={{ borderColor: W.border, borderTopColor: W.red }}
            />
          </div>
        ) : error ? (
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(155deg, ${preview.accentColor}1f 0%, #0a0202 80%)`,
                }}
              >
                {preview.templateType === "video" && preview.previewVideoUrl ? (
                  ...
                  <span className="text-[11px] py-16" style={{ color: W.dim }}>
                    Preview generating…
                  </span>
                )}
                {preview.isPro && (
                  <div
                    className="absolute top-3 left-3 flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/70 backdrop-blur-md"
                ...
                <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar px-5 sm:px-6 pt-5 sm:pt-6">
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="min-w-0">
                      ...
                      <p className="text-sm mt-0.5" style={{ color: W.muted }}>
                        {preview.description}
                      </p>
                    </div>
                    <span
                      className="mt-1 shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full"
                      style={{
                        background: `${preview.accentColor}20`,
                        color: preview.accentColor,
                        border: `1px solid ${preview.accentColor}35`,
                      }}
                    >
                      {preview.category}
                    </span>
                  ...
                  {preview.isPro && (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="font-semibold underline underline-offset-2 cursor-pointer hover:text-white transition-colors"
                        style={{ color: W.muted }}
                      >
                        Upgrade to Pro
                      </button>
